...
class InvoiceController extends Controller
{
    public function Home(){
        ...
        return view('pages.invoices.v_home',$data);
    }
    public function Create(Request $request){
        ...
        $data['tittle'] = 'Porting - Dashboard';
        $insert = DB::table('l_invoices')
                    ->insert([
                        'quotation_id' => $q_id,
                        'invoice_date' => $start,
                        'invoice_due_date' => $expired,

                    ]);
        $q= DB::table('l_invoices')
                    ->orderBy('quotation_id','DESC')
                    ->limit(1)
                    ->first();
        $d_id = $q;
    ...
    public function Edit($id){
        $get = DB::table('l_invoices')
                ->where('invoice_id',$id)
                ->first();
        $id_q = $get->quotation_id;
        $data['quotation_head'] = DB::table('l_quotations')
                                ...
                                ->join('l_invoices','l_invoices.quotation_id','l_quotations.quotation_id')
                                ->where('l_quotations.quotation_id',$id_q)
                                ->first();
        ...
        return view('pages.invoices.v_h_invoice_create',$data);
    }
